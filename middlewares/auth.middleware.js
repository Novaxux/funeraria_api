...
export function requireRole(expectedRoles) {
	...
	const roles = Array.isArray(expectedRoles) ? expectedRoles.map(Number) : [Number(expectedRoles)];
	return (req, res, next) => {
		const user = req.user;
		if (!user) return res.status(401).json({ error: "Unauthorized" });
		const roleId = Number(user.role_id || -1);
		if (!roles.includes(roleId)) return res.status(403).json({ error: "Forbidden" });
		return next();
	};
}
